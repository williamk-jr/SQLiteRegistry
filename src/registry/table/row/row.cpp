...
namespace iamaprogrammer {
  Row::Row(sqlite3* db, std::string table, std::string key, TableSchema* schema): schema(schema), hasOwnership(true) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &this->statement, nullptr) != SQLITE_OK) {
      std::cerr << "Failed to prepare delete statement for table '" << table << "': " << sqlite3_errmsg(db) << std::endl;
      return;
    }

    sqlite3_bind_text(this->statement, 1, key.c_str(), -1, SQLITE_TRANSIENT);
    if (sqlite3_step(this->statement) != SQLITE_ROW) {
      std::cerr << "Failed to retrieve entry from '" << table << "': " << sqlite3_errmsg(db) << std::endl;
    }
  }

  Row::Row(sqlite3* db, std::string table, int key, TableSchema* schema): schema(schema), hasOwnership(true) {
    std::string pk = this->schema->get(0).first;
    std::string sql = "SELECT * FROM "+table+" WHERE "+pk+" = ?1;";
  ...
  Row::Row(sqlite3* db, std::string table, double key, TableSchema* schema): schema(schema), hasOwnership(true) {
    std::string pk = this->schema->get(0).first;
    std::string sql = "SELECT * FROM "+table+" WHERE "+pk+" = ?1;";
